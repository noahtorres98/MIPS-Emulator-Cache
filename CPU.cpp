...
void CPU::decode() {
    ...
    switch(opcode) {
        ...
        case 0x03: D(cout << "jal " << hex << ((pc & 0xf0000000) | addr << 2)); // P1: pc + 4
            ...
            aluSrc1 = pc;
            aluSrc2 = regFile[REG_ZERO]; stats.registerSrc(REG_ZERO, EXE1);// always reads zero
            pc = (pc & 0xf0000000) | addr << 2;
            stats.flush(2);
            break;
...
void CPU::printFinalStats() {
    ...
    //    cout << "RAW hazards: " << stats.getRaw() << " (1 per every " << fixed << setprecision(2) << 1.00 *  instructions/stats.getRaw() << " instructions)" <<  endl;
    //    cout << "  On EXE1 op: " << stats.getRawEXE1() << " (" << fixed << setprecision(0) << 100.0 * stats.getRawEXE1()/stats.getRaw() << "%)" << endl;
    //    cout << "  On EXE2 op: " << stats.getRawEXE2() << " (" << 100.0 * stats.getRawEXE2()/stats.getRaw() << "%)" << endl;
    //    cout << "  On MEM1 op: " << stats.getRawMEM1() << " (" << 100.0 * stats.getRawMEM1()/stats.getRaw() << "%)" << endl;
    //    cout << "  On MEM2 op: " << stats.getRawMEM2() << " (" << 100.0 * stats.getRawMEM2()/stats.getRaw() << "%)" << endl;
}
